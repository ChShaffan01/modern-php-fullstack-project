...
if (isset($_SESSION['user_id']) && isset($_SESSION['user_name'])) {
    ...
    // Check if admin (assuming you store this in session)
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin') {
        $isAdmin = true;
    }
    
    // If Auth class exists, use it for additional checks
    if (class_exists('Auth')) {
        try {
            require_once 'db.php'; // Make sure this path is correct
            $auth = new Auth($pdo);
        } catch (Exception $e) {
            // Silent fail - use session data instead
        }
    }
}
?>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <?php if ($isLoggedIn): ?>
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="results.php"><i class="fas fa-chart-bar me-1"></i>My Results</a>
                        </li>
                        <?php if ($isAdmin): ?>
                            <li class="nav-item">
